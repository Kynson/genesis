# User configurations

{
  # Admin user
  users.users.kynsonszetau = {
    description = "Kynson Szetau";
    isNormalUser = true;
    extraGroups = [ "wheel" "networkmanager" ];
    uid = 1000;
    hashedPassword = "$y$j9T$JAVeq6.kjto6cbNEAeuqS1$92mniU80dNIx58TdJ82LlCd/J7zAw5RO5mhMlQu/DD5";
    openssh.authorizedKeys.keys = [
      "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIGpoAKthyd4To/65waKo7qg6D92CJV1VN+o01eMAqXGJ"
      "ssh-rsa "
    ];
  };

  # Worker user
  users.users.genesis = {
    description = "Genesis Containers Worker";
    isNormalUser = true;
    uid = 1001;
    # Enable lingering to allow user units to be started at boot
    linger = true;
  };
}